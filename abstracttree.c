     ...
     case NODETYPE_AND   : if(!(strcmp(ptr1->type->name,"boolean") == 0 && strcmp(ptr1->type->name,"boolean") == 0)){
      ...
      case NODETYPE_FUNC :
           ...
           if(t) {
      		while(t && list) {
               	if(list->passType==2) {
               		if(t->ptr2->node!=NODETYPE_ID && t->ptr2->node!=NODETYPE_FIELD_END) {
...
void check_type(struct ASTNode * id) {
	   struct Gsymbol *temp1=GLookup(id->name);
      	    ...
      	    if(temp2) {
      	    	 ...
      	    	 if(strcmp(temp2->type->name,"integer")!=0 &&  strcmp(temp2->type->name,"boolean")!=0) {
                   		...
                   		if(temp2->size>1) {
                   			printf("%s is a variable  ,it is not a user defined type \n",id->name);
   			 			          exit(1);
                   		}
                }
              ...
              id->type=temp2->type;
      	    	...
      	    	if(strcmp(temp1->type->name,"integer")!=0 &&  strcmp(temp1->type->name,"boolean")!=0) {
                   		...
                   		if(temp1->size>1) {
                   			printf("%s is not a variable  ,it is  not userdefined type \n",id->name);
   			 			    exit(1);
                   		}
                   	}
...
void check_type_array(struct ASTNode * ida) {
